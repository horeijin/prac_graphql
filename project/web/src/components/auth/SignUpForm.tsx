...
const SignUpRealForm = () => {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<SignUpMutationVariables>(); //회원가입 뮤테이션의 타입 지정
  
  const navigate = useNavigate(); //홈 이동 위해 사용
    ...
    <Stack as="form" spacing={4} onSubmit={handleSubmit(onSubmit)}>
      <FormControl isInvalid={!!errors.signUpInput?.email}>
        ...
        <Input type="email" placeholder='example@example.com' {...register('signUpInput.email', {
            ...
            pattern: {
              value:
                // 아래의 주석은 ESLint의 max-len 규칙을 다음 라인에서만 무시한다는 의미
      ...
      <FormControl isInvalid={!!errors.signUpInput?.password}>
        ...
        <Input type="password" placeholder='8자 이상의 영문, 숫자, 특수문자' {...register('signUpInput.password', {
            required: '암호를 입력해주세요.',
